...
export default function TaskForm({ subjects, onAdd }) {
  ...
  const fieldCls = 'flex-1 min-w-0 bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-white rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500 placeholder-slate-400'
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2 bg-white dark:bg-slate-800 rounded-2xl p-4 border border-slate-100 dark:border-transparent">
      ...
      <button type="button" onClick={() => setExpanded(v => !v)}
        className="text-xs text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-400 transition-colors text-left">
