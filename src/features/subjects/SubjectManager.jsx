...
export default function SubjectManager({ subjects, onAdd, onRemove, onSelect }) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-sm mx-auto">
      <h2 className="text-lg font-bold text-white">Derslerim</h2>

      <form onSubmit={handleSubmit} className="flex flex-col gap-3 bg-slate-800 rounded-2xl p-4">
        <input
          type="text"
          value={name}
          ...
          maxLength={40}
          className="bg-slate-700 text-white rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-emerald-500 placeholder-slate-400"
        />
        ...
        <button
          type="submit"
          className="bg-emerald-500 hover:bg-emerald-400 text-white font-semibold rounded-lg px-4 py-2 text-sm transition-colors"
        >
          Ders Ekle
        </button>
      </form>
      ...
      {subjects.length === 0 ? (
        <p className="text-slate-500 text-sm text-center py-4">Henüz ders eklenmedi.</p>
      ) : (
