...
function AddForm({ onAdd }) {
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)}
      className="text-sm text-emerald-500 hover:text-emerald-400 transition-colors text-left">
      + Video Ekle
    </button>
  )

  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2 bg-slate-100 dark:bg-slate-700 rounded-xl p-3">
...
function VideoItem({ video, isFirst, isLast, onToggle, onMove, onEdit, onRemove }) {
  const btnCls = 'w-6 h-6 flex items-center justify-center text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors disabled:opacity-20'
  return (
    <div className={`flex items-center gap-2 px-3 py-2.5 rounded-xl ${video.completed ? 'bg-slate-100/60 dark:bg-slate-700/40' : 'bg-slate-100 dark:bg-slate-700'}`}>
      <button onClick={() => onToggle(video.id)}
        className={`w-5 h-5 rounded-full border-2 shrink-0 flex items-center justify-center transition-colors ${
          video.completed ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 dark:border-slate-500 hover:border-emerald-400'
        }`}>
        {video.completed && (
      ...
      <a href={video.url} target="_blank" rel="noopener noreferrer"
        className={`flex-1 text-sm min-w-0 truncate transition-colors ${
          video.completed ? 'line-through text-slate-400 dark:text-slate-500' : 'text-slate-900 dark:text-white hover:text-emerald-500'
        }`}>
        {video.title}
      </a>
      ...
      <div className="flex items-center shrink-0">
        ...
        <button onClick={() => onRemove(video.id)}
          className="w-7 h-6 flex items-center justify-center text-slate-300 dark:text-slate-600 hover:text-red-500 transition-colors text-xl leading-none" title="Sil">×</button>
      </div>
    </div>
  )
}
...
export default function SubjectVideos({ videos, onAdd, onUpdate, onRemove, onToggle, onMove }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {videos.length === 0
        ? <p className="text-slate-400 dark:text-slate-600 text-sm py-1">Henüz video eklenmedi.</p>
        : (
