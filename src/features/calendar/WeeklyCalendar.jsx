...
export default function WeeklyCalendar({ weekDays, sessions, subjects, onDeleteSession }) {
  ...
  return (
    <div className="overflow-x-auto -mx-4">
      <div style={{ minWidth: '420px' }} className="px-4">
        {/* Day headers */}
        <div className="flex border-b border-slate-700 mb-0 sticky top-0 bg-slate-900 z-10">
            ...
            return (
              <div key={toDateStr(day)} className="flex-1 text-center py-2">
                ...
                <div className={`text-sm font-bold mx-auto w-7 h-7 flex items-center justify-center rounded-full ${
                  isToday ? 'bg-emerald-500 text-white' : 'text-white'
                }`}>
                  {day.getDate()}
                </div>
              </div>
        ...
        <div className="flex" style={{ height: totalHeight }}>
          ...
          {weekDays.map(day => {
            ...
            return (
              <div key={dayStr} className="flex-1 relative border-l border-slate-800">
                {hours.map((_, i) => (
                  <div key={i} style={{ top: i * HOUR_HEIGHT }}
                    className="absolute w-full border-t border-slate-800 pointer-events-none" />
                ))}
